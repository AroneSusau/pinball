...
Cylinder::Cylinder(glm::vec2 uv, float baseRadius, float topRadius, float height, float stack, float sector)
	: uv(uv)
...
void Cylinder::Generate()
{
    ...
    for (int i = 0, j = 0; i < sectorCount; ++i, j += 3)
    {
        ...
        v.push_back(glm::vec3(x * baseRadius, y * baseRadius, z));
        n.push_back(glm::vec3(0, 0, -1));
    ...
    for (int i = 0, j = 0; i < sectorCount; ++i, j += 3)
    {
        x = unitVertices[j];
        y = unitVertices[j + 1];
        v.push_back(glm::vec3(x * topRadius, y * topRadius, z));
        n.push_back(glm::vec3(0, 0, 1));
    ...
    for (int i : indices)
    {
        ...
        texels.push_back(glm::vec2(rotate * glm::vec3(tv.at(i), 1.0)));
    }
}